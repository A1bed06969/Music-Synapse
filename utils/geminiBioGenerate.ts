...
執筆ルール:
- 日本語で150〜200字程度を目安にする(情報量に応じて多少前後してよい)
- 文体・トーンの参考(内容の分量や具体性をそのまま真似する必要はありません):
  「${EXAMPLE_BIO}」
- 見出しや箇条書きは使わず、地の文のみ
- 文末は「〜である」を使わない。体言止めや「〜している」「〜集めている」など、上記の文体見本に近い、硬すぎない結び方にする
- ジャンルの箇条書き的な列挙で文章を埋めない。データベース上の既知の事実にあるジャンルは、書くとしても代表的なもの1つ程度を文章の流れの中で自然に触れるに留め、複数タグをそのまま並べる書き方はしないでください。ソーステキストに具体的な経歴・エピソードがあるなら、そちらを主軸にしてください
- 上記の情報に含まれる具体的な経歴やエピソードが乏しく、憶測や一般的な表現で埋めないと紹介文として成立しない場合は、無理に書かずstatusを"INSUFFICIENT"にしてください(文字数の少なさそのものではなく、書ける具体的な事実があるかどうかで判断してください)
