...
import { updateEventAppearance, deleteEventAppearance } from '../../../actions'
...
export default async function EditEventAppearancePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: entry, error }, { data: artists }, { data: eventEditions }] = await Promise.all([
    supabase
      .from('event_appearance')
      .select('id, event_edition_id, artist_id, stage, venue, start_time, end_time, is_headliner')
      .eq('id', id)
      .single(),
