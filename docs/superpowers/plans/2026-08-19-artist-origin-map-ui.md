...
- The `geo_boundary` repair pass (`scripts/backfill-artist-origin-geo-codes.ts`, from the data-layer plan) should be re-run periodically as new artists get geocoded, so newly-added artists' municipality/region polygons are ready before this UI needs them. Post-fix-wave behavior: an artist with a resolved `origin_country_code` but no cached region/municipality polygon yet now correctly renders as a point marker within their country's view (finding #3 of the final review fixed the `fallbackMarkers` filter that previously missed this case). An artist with no `origin_country_code` at all yet (e.g. added after the last backfill run) is not reachable via the drill-down UI at all until the next backfill run resolves their country — they remain visible only in the side-list panel until then.
