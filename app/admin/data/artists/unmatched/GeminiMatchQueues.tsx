...
import { confirmGeminiMatchLog, revertGeminiMatchLog } from './geminiMatchActions'
...
export function GeminiReviewQueue({ rows }: { rows: ReviewLogRow[] }) {
  ...
  function handleConfirm(id: string) {
    ...
    startTransition(async () => {
      const result = await confirmGeminiMatchLog(id)
      if (result.success) {
  ...
  return (
    <div className="mt-6">
      ...
      <p className="mt-1 text-xs text-white/40">Geminiが選んだ候補です。合っていれば確定、違えば下の検索欄から手動で探してください。</p>
          ...
          <li key={r.id} className="rounded-md border border-white/10 p-3 text-sm">
            <div className="flex flex-wrap items-center gap-2">
              {r.imageUrl ? (
                ...
                <img src={r.imageUrl} alt="" className="h-8 w-8 shrink-0 rounded-full object-cover" />
              ) : (
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/10 text-[10px] text-white/30">?</span>
              )}
              <span className="font-medium">{r.stubArtistName}</span>
              <span className="text-white/30">→</span>
              <span>{r.chosenArtistName}</span>
              ...
              >
                {isPending && pendingId === r.id ? '確定中...' : '✓ この人で確定'}
              </button>
            </div>
...
export function GeminiAutoAppliedList({ rows }: { rows: AutoAppliedLogRow[] }) {
  ...
  function handleRevert(id: string) {
    setPendingId(id)
    startTransition(async () => {
      const result = await revertGeminiMatchLog(id)
      if (result.success) {
  ...
  return (
    <div className="mt-8">
      ...
      <p className="mt-1 text-xs text-white/40">確信度90%以上でGeminiが自動反映したものです。間違っていれば取消できます。</p>
      <ul className="mt-3 flex flex-col gap-2">
        {visible.map((r) => (
