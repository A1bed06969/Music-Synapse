...
export default async function EventsPage({
  ...
  function groupByContinentAndGenre(targetRows: EventRow[]) {
  ...
  const categorySections = [
    { label: 'フェス・イベント', continentSections: groupByContinentAndGenre(festivalRows) },
    { label: 'ツアー', continentSections: groupByContinentAndGenre(tourRows) },
  ].filter((c) => c.continentSections.length > 0)
    ...
    <div className="mx-auto max-w-[1600px] px-6 py-12">
      ...
      {categorySections.length === 0 ? (
        ...
        <div className="mt-12 space-y-14">
          {categorySections.map(({ label, continentSections }) => (
            <section key={label}>
                  ...
                  <section key={continent}>
                    ...
                    <div className="mt-4 space-y-6">
                      {genres.map(([genreName, genreEvents]) => (
                        <div key={genreName}>
                          <h4 className="text-xs font-medium uppercase tracking-wide text-white/40">{genreName}</h4>
