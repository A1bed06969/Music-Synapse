...
A few things the follow-up plan's author should know going in:

- `scripts/backfill-artist-origin-geo-codes.ts` needs to be re-run periodically as new artists get geocoded — it's idempotent/incremental (only processes artists with `origin_country_code IS NULL`), and it now also runs a repair pass after the main loop that re-attempts fetching any `geo_boundary` row missing for an already-resolved artist (covers transient failures on the earlier run; costs nothing once everything is cached).
- GB (England/Scotland/Wales/Northern Ireland) and some French region codes are **permanently** polygon-less: Nominatim returns a home-nation-level ISO 3166-2 code for the UK, but Natural Earth's 10m admin-1 dataset only has finer county/unitary-authority-level `GB-*` codes, never `GB-ENG`/`GB-SCT`/etc. literally — this is a genuine data-source granularity mismatch, not a bug, and it will never resolve itself no matter how many times the backfill or repair pass runs. The map UI must treat "region/muni code is set on the artist but no matching `geo_boundary` row exists" as a further fallback case (fall back to the country block), never assume every non-null `origin_region_code`/`origin_muni_code` has a renderable polygon, and never treat it as an error to fix.
- `geo_boundary` is publicly readable (RLS: `SELECT` open to `public`) and already ~4MB across cached rows, with some individual municipality/region geometries as large as ~600KB. The map UI should query only the specific `level`/`code` rows it needs to render a given view — never `select *` the whole table.
